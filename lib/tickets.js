...
export function buildTiketFlowWhatsAppMessage({ ticket, event, ticketType, isFree = false }) {
  ...
  const cardUrl = `${SITE_URL}/api/tickets/card/${ticket?.qr_code_token || ''}`;
  ...
  return [
    ...
    '',
    // WhatsApp renders a direct image URL as an inline photo preview, so the
    // buyer sees their actual ticket card with the QR code in the chat.
    'Your ticket card 👇',
    cardUrl,
    '',
    'Tap to open your digital ticket:',
    ticketUrl,
    '',
...
export async function sendTicketWhatsApp({ ticket, event, ticketType, isFree = false }) {
  ...
  if (token && phoneNumberId && to) {
    ...
    // Send the ticket card as an image when possible, falling back to text.
    const sendMedia = async () => {
      try {
        const cardUrl = `${SITE_URL}/api/tickets/card/${ticket?.qr_code_token || ''}`;
        const mediaRes = await fetch(`https://graph.facebook.com/v19.0/${phoneNumberId}/media`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({ messaging_product: 'whatsapp', url: cardUrl, type: 'image' }),
        });
        ...
        await fetch(`https://graph.facebook.com/v19.0/${phoneNumberId}/messages`, {
          ...
          body: JSON.stringify({
            ...
            type: 'image',
            image: { id: mediaId },
            caption: `🎟️ Your TiketFlow ticket for ${event?.event_name || 'your event'} — tap to open your digital ticket 👇\n${SITE_URL}/ticket/${ticket?.qr_code_token || ''}`,
          }),
        });
      } catch (err) {
        // Fall back to a text message if media upload fails
        await fetch(`https://graph.facebook.com/v19.0/${phoneNumberId}/messages`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
    ...
    sendMedia().catch(err => console.error('WhatsApp send error:', err));
  }
        ...
        <div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:520px;margin:0 auto;padding:24px;">
          ...
          <div style="border:1px solid #e5e7eb;border-top:none;border-radius:0 0 16px 16px;padding:24px;">
            ...
            <div style="margin:24px 0 12px;text-align:center;">
              <img src="${cardUrl}" alt="Your TiketFlow ticket card" style="max-width:100%;border-radius:14px;border:1px solid #e5e7eb;" />
            </div>
