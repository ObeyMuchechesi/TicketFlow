...
export async function requireRole(req, ...roles) {
  ...
  let { data: user } = await supabase
    .from('users')
    .select('id, role, is_active, assigned_event_id')
    .eq('id', session.userId)
    .single();

  // DB not yet migrated with assigned_event_id — retry without it.
  // assignedEventId stays null, which FAILS CLOSED for gate staff: until the
  // migration runs they see no events and can scan no events (never all).
  if (!user) {
  ...
  return { userId: user.id, role: user.role, assignedEventId: user.assigned_event_id || null };
}
